...
function FranchiseeApp({ token, onLogout }) {
  ...
  async function finalizeSave({
    ...
    const saved = await saveInvoiceToServer({
      ...
      discount: Math.min(Math.max(0, Math.round(num(discountInr))), maxDiscount),
      installation_fee: Math.max(0, Math.round(num(installationFeeInr))),
      tax_mode: mode,
